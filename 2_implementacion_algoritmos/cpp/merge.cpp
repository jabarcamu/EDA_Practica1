...
// Algoritmo de ordenamiento MERGE SORT - divide y conquistarás
...
// arr, arreglo de números
// l, índice del límite inferior del arreglo
// m, índice medio entre el límite inferior l y superior r, del arreglo
// r, índice del límite superior del arreglo
void merge(int arr[], int l, int m, int r)
{
	int n1 = m - l + 1; // cantidad de elementos del arreglo izquierdo.
	int n2 = r - m; // cantidad de elementos del arreglo derecho.

	// creación de los arreglos temporales izquierdo y derecho respectivamente.
	int L[n1], R[n2];
...
// l, índice inferior del arreglo
// r, índice superior del arreglo
void mergeSort(int arr[],int l,int r){
	if(l>=r){
		return; // retorno de la recursividad(paso base)
	}
	int m =l+ (r-l)/2; // calculando el índice del punto medio del arreglo
...
int main()
{
    ...
    // archivo para alamacenar los tiempos de ejecución
    ofstream myfile;
    myfile.open ("mergeSort_cpp_time.txt");

    for (int k = 0; k < generate.size(); k++)
    {
        // carga de números alearios generados para cada caso de prueba       
        ifstream infile2("../../1_preparacion_datos/generatedTestData/example_" + to_string(generate[k]) + ".txt");
        
        int arr[generate[k]]; // creando el arreglo para los n valores de números
        int i=0;
        string line;
        ...
        // calculando el tiempo de ejecución del algoritmo.
        clock_t start, end;
        start = clock();    
        mergeSort(arr, 0, n - 1);
        end = clock();
        ...
        // guardando resultados de ejecución del algoritmo para cada caso de prueba
        myfile << generate[k] << " "; 
